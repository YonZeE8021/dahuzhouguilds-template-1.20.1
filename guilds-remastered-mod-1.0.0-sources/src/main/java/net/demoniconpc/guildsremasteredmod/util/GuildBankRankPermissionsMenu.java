...
public class GuildBankRankPermissionsMenu {
    ...
    public static void openForPlayer(ServerPlayerEntity player, String rankName) {
        ...
        inventory.setStack(53, GuildPermissionsMenu.menuItem((Item)Items.field_8107, (String)"Back", null).icon.copy());
    ...
    private static ItemStack toggleItem(String permission, boolean enabled) {
        ItemStack stack = new ItemStack((ItemConvertible)(enabled ? Items.field_8131 : Items.field_8298));
        stack.set(DataComponentTypes.field_49631, (Object)Text.literal((String)((enabled ? "\u00a7a" : "\u00a77") + GuildBankRankPermissionsMenu.getLabel(permission))));
        return stack;
    }
        ...
        public void onSlotClick(int slot, int button, SlotActionType actionType, PlayerEntity player) {
            if (actionType == SlotActionType.PICKUP && player instanceof ServerPlayerEntity) {
                ...
                for (int i = 0; i < PERMISSION_SLOTS.length; ++i) {
                    ...
                    boolean currentlyEnabled = this.menuInventory.getStack(slot).getItem() == Items.field_8131;
